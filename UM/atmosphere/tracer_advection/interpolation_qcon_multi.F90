...
! Subroutine Interpolation_qcon_multi.
...
SUBROUTINE interpolation_qcon_multi(                                           &
  ...
  group_2dcomm,size_int_qc_mult,proc_all_group,                                &
...
INTEGER (KIND=integer32) :: i_store(size_int_qc_mult,0:size_2dcomm)
INTEGER (KIND=integer32) :: j_store(size_int_qc_mult,0:size_2dcomm)
INTEGER (KIND=integer32) :: k_store(size_int_qc_mult,0:size_2dcomm)
INTEGER (KIND=integer32) :: i_out_e(size_int_qc_mult)
INTEGER (KIND=integer32) :: j_out_e(size_int_qc_mult)
INTEGER (KIND=integer32) :: k_out_e(size_int_qc_mult)
INTEGER (KIND=integer32) :: i_out_w_e(size_int_qc_mult)
INTEGER (KIND=integer32) :: j_out_w_e(size_int_qc_mult)

INTEGER :: n_sendto(0:size_2dcomm)
INTEGER :: n_recvfrom(0:size_2dcomm)
INTEGER :: k_level_e(size_int_qc_mult)
INTEGER :: isend_arr(5,size_int_qc_mult,0:size_2dcomm)
INTEGER :: irecv_arr(5,size_int_qc_mult,0:size_2dcomm)
...
REAL :: rsend_arr(5,size_int_qc_mult,0:size_2dcomm)
REAL :: rrecv_arr(5,size_int_qc_mult,0:size_2dcomm)
REAL :: weight_lambda_e(size_int_qc_mult)
REAL :: weight_phi_e(size_int_qc_mult)
REAL :: weight_lambda_w_e(size_int_qc_mult)
REAL :: weight_phi_w_e(size_int_qc_mult)
REAL :: data_out_high_e(size_int_qc_mult*number_of_inputs)
REAL :: data_out_mono_e(size_int_qc_mult*number_of_inputs)
REAL :: send_data(2*number_of_inputs*size_int_qc_mult,0:size_2dcomm)
REAL :: recv_data(2*number_of_inputs*size_int_qc_mult,0:size_2dcomm)
REAL :: coeff_z_e(size_int_qc_mult, -2:3)
REAL :: coeff_z_lin_e(size_int_qc_mult, 0:1)
REAL :: r_out_e(size_int_qc_mult)
...
TYPE (sendrecv_type) :: send_array(size_int_qc_mult,0:size_2dcomm)
TYPE (sendrecv_type) :: recv_array(size_int_qc_mult,0:size_2dcomm)
...
IF (lhook) CALL dr_hook('INTERPOLATION_QCON_MULTI',zhook_in,zhook_handle)
...
!$OMP  PARALLEL DO SCHEDULE(STATIC) DEFAULT(SHARED) PRIVATE(icnt, inx, itmp,   &
!$OMP& irecv, i, j, k, ic)
        DO k = 1, dim_k_out
          DO j = j0, j1
            icnt=0
...
!$OMP END PARALLEL DO 
        ...
        IF (dim_e_out  >   size_int_qc_mult      ) THEN
          errorstatus = 10
          
          CALL ereport("Interpolation_qcon_multi", ErrorStatus,                &
      ...
      IF ( model_domain == mt_global ) THEN
        IF (pole_handling  ==  0) THEN
          ...
          IF (at_extremity(pnorth)) THEN
            DO k = 1, dim_k_out
              DO i = 1, dim_i_out
                i_out(i,dim_j_out,k) = i + dsm1x
                i_out_w(i,dim_j_out,k) = i + dsm1x
          ...
          IF (at_extremity(pnorth)) THEN
            DO k = 1, dim_k_out
              DO i = 2, dim_i_out
                i_out(i,dim_j_out,k) = i + dsm1x
                i_out_w(i,dim_j_out,k) = i + dsm1x
                j_out(i,dim_j_out,k) = dsm1y + dim_j_out
                j_out_w(i,dim_j_out,k) = dsm1y + dim_j_out
              END DO
            END DO
          END IF
  ...
  IF ( l_high .AND. l_mono .AND. l_conserv ) THEN
    ...
    DO n=1,number_of_inputs
      ...
      CALL mono_conserv_qcon (                                                 &
        ...
        ext_data(1-halo_i,1-halo_j,-1,n),                                      &
...
! End of routine.
IF (lhook) CALL dr_hook('INTERPOLATION_QCON_MULTI',zhook_out,zhook_handle)
RETURN
END SUBROUTINE interpolation_qcon_multi
