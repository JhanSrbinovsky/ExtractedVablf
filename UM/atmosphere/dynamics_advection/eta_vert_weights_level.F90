...
! start of SWW mods
      Integer M,MM
#if defined(IBM)
      Real  r_in (1-halo_i:1-halo_i,0:(dim_i_in+2*halo_i)*              &
     &    (dim_j_in+2*halo_j)-1,dim_k_in) 
#else
...
#include "monots.h"
      ...
      Integer ijlistup(dim_i_out*dim_j_out)
      Integer ijlistdn(dim_i_out*dim_j_out),ix
      Integer cnt
...
! start of SWW mods
#if defined(IBM)
! M is number of elements skipped over by an offset of "1" in "J" dim
! MM is offset within "adjusted" R_in array for R_in(1,1,index)
       M =dim_i_out + 2*halo_i
       MM=halo_j*(dim_i_out + 2*halo_i) 
#else
       M=1
       MM=1
#endif
...
! loops collapsed manually
! Initialisation of lists ijlistup and ijlistdn
            j=1
            Do i = 1, dim_i_out*dim_j_out
#endif
...
#else
! loops collapsed manually + logical search replaced by
! list processing
                j=1
!CDIR NODEP
                Do ix = 1, count_down
...
#else
! cjs 110200 Obtain an eta coordinate for the departure point.
                weight_eta = ( r_out(i,j,k) - r_here ) /                &
     &                       ( r_here_plus - r_here )
