...
REAL ::                         & ! 
  rout2d(1,1,wet_levels)    ! Full field
...
IF (L_SCMDiags(SCMDiag_conv) .and. l_dilute ) THEN
  DO k= 1,wet_levels
    rout2d(1,1,k)=ql_parc(1,k)
  END DO
! DEPENDS ON: scmoutput
  Call SCMoutput(rout2d,'ql_parc','parcel water                ',          &
                       'kg/kg', t_inst,d_wet,default_streams,'',routinename)
  DO k= 1,wet_levels
    rout2d(1,1,k)=ql_parc_dil(1,k)
  END DO
! DEPENDS ON: scmoutput
  ...
  DO k= 1,wet_levels
    rout2d(1,1,k)=sl_parc(1,k)
  END DO
! DEPENDS ON: scmoutput
  Call SCMoutput(rout2d,'sl_parc','Parcel energy          ','K',           &
                       t_inst,d_wet,default_streams,'',routinename)

  DO k= 1,wet_levels
    rout2d(1,1,k)=qw_parc(1,k)
  END DO
! DEPENDS ON: scmoutput
  Call SCMoutput(rout2d,'qw_parc','total parcel water          ',          &
                       'kg/kg',t_inst,d_wet,default_streams,'',routinename)
