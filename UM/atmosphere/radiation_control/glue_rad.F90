...
! DEPENDS ON: scmoutput
        CALL scmoutput(work_3d,                                         &
             'dt_swpc2','SW heating rate incl PC2','K/timestep',        &
...
!     Equiv to stash 1,161
      CALL scmoutput(t_incr_diagnostic,                                 &
           'dt_sw','SW heating rate','K/timestep',                      &
           t_acc,d_all,default_streams,'',routinename)
      CALL scmoutput(t_incr_diagnostic,                                 &
      ...
      IF (l_surfsw_cor) CALL scmoutput(surfsw_cor,                      &
           'surfsw_cor','Corrected net surface SW flux','W/m2',         &
           t_avg,d_sl,default_streams,'',routinename)
      IF (l_toasw_cor) CALL scmoutput(toasw_cor,                        &
           'toasw_cor','Corrected outgoing SW flux (TOA)','W/m2',       &
           t_avg,d_sl,default_streams,'',routinename)
      IF (l_surfdir_cor) CALL scmoutput(surfdir_cor,                    &
           'surfdir_cor','Corrected direct surface SW flux','W/m2',     &
           t_avg,d_sl,default_streams,'',routinename)
      IF (l_surfdif_cor) CALL scmoutput(surfdif_cor,                    &
           'surfdif_cor','Corrected diffuse surface SW flux','W/m2',    &
           t_avg,d_sl,default_streams,'',routinename)
...
!     stash 1,224
      CALL scmoutput(sw_diag(j_sw)%lwp_strat,                           &
           'lwp_strat','Layer cld liquid water path * weight','kg/m2',  &
           t_avg+only_radsteps,d_cloud,default_streams,'',routinename)
!     stash 1,225
...
!       stash item = 2,181  ! temperature increment minus pc2
        CALL scmoutput(work_3d,                                         &
             'dt_lwpc2','LW heating rate incl PC2','K/timestep',        &
             t_acc,d_all,default_streams,'',routinename)
        CALL scmoutput(work_3d,                                         &
...
!     stash 2,161
      CALL scmoutput(t_incr_diagnostic,                                 &
           'dt_lw','LW heating rate minus PC2','K/timestep',            &
           t_acc,d_all,default_streams,'',routinename)
      CALL scmoutput(t_incr_diagnostic,                                 &
...
!     stash 2,192  total cloud fraction increment
      ...
      END DO     ! k
      CALL scmoutput(work_3dw,                                          &
           'dbcf_lw','bulk cloud fraction increment lwrad',             &
           'fraction',                                                  &
           t_avg+only_radsteps,d_wet,default_streams,'',routinename)
