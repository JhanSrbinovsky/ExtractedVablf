...
#if defined(SCMA)
#include "s_scmop.h"
  CHARACTER(LEN=*), PARAMETER ::  RoutineName = 'kmkhz'
#endif
...
#if defined(SCMA)
  REAL ::                                                               &
   scmout(pdims%i_start:pdims%i_end,pdims%j_start:pdims%j_end),         &
   scmout2(pdims%i_start:pdims%i_end,pdims%j_start:pdims%j_end),        &
   scmout3(pdims%i_start:pdims%i_end,pdims%j_start:pdims%j_end,         &
           rkmdims%k_end+1)
                                                   ! For SCM diagnostics
#endif
...
!$OMP DO SCHEDULE(STATIC)
    DO j = pdims%j_start, pdims%j_end
      DO i = pdims%i_start,pdims%i_end
        IF ( dsc(i,j) ) THEN
          scmout(i,j)= d_siems_dsc(i,j)
        ELSE
          scmout(i,j)= d_siems(i,j)
        END IF
      END DO
    END DO
!$OMP END DO
...
!$OMP DO SCHEDULE(STATIC)
    DO j = pdims%j_start, pdims%j_end
      DO i = pdims%i_start,pdims%i_end
        ...
        IF ( ABS(dqw_sml(i,j)) < SQRT(TINY(1.0)) ) THEN
          scmout(i,j)= rmdi
        ELSE
          scmout(i,j)= 1.0 + dsl_sml(i,j)/(lcrcp*dqw_sml(i,j))
        END IF

        IF ( dsc(i,j) ) THEN
          IF ( ABS(dqw_dsc(i,j)) < SQRT(TINY(1.0)) ) THEN
            scmout2(i,j)= rmdi
          ELSE
            scmout2(i,j)= 1.0 + dsl_dsc(i,j)/(lcrcp*dqw_dsc(i,j))
          END IF
        ELSE
          scmout2(i,j)= scmout(i,j)
        END IF
...
!$OMP DO SCHEDULE(STATIC)
    DO j = pdims%j_start, pdims%j_end
      DO i = pdims%i_start,pdims%i_end
        IF ( dsc(i,j) ) THEN
          scmout(i,j)= chi_s_dsct(i,j)
        ELSE
          scmout(i,j)= chi_s_top(i,j)
        END IF
      END DO
    END DO
!$OMP END DO
...
!$OMP DO SCHEDULE(STATIC)
    DO j = pdims%j_start, pdims%j_end
      DO i = pdims%i_start,pdims%i_end
        IF ( dsc(i,j) ) THEN
          scmout(i,j)= db_dsct(i,j)
        ELSE
          scmout(i,j)= db_top(i,j)
        END IF
      END DO
    END DO
!$OMP END DO
...
!------------------------------------------------------
! DEPENDS ON: scmoutput
    CALL SCMoutput(qcl_ic_top,                                          &
         'QCL_IC_TOP','BL-top in-cloud water content','',               &
         t_avg,d_sl,default_streams,'',RoutineName)
!------------------------------------------------------
! DEPENDS ON: scmoutput
    CALL SCMoutput(df_top_over_cp,                                      &
...
!$OMP DO SCHEDULE(STATIC)
    DO k = 1,bl_levels+1
      DO j = pdims%j_start, pdims%j_end
        DO i = pdims%i_start,pdims%i_end
          scmout3(i,j,k)= frad_lw(i,j,k)
        END DO
      END DO
    END DO
!$OMP END DO
!$OMP DO SCHEDULE(STATIC)
! need to use rkmdims%k_end(=bl_levels-1) here because bl_levels
! in this routine is actually nl_bl_levels (< bl_levels)
    DO k = bl_levels+2,rkmdims%k_end+1
      DO j = pdims%j_start, pdims%j_end
        DO i = pdims%i_start,pdims%i_end
          scmout3(i,j,k)= 0.0
        END DO
...
! DEPENDS ON: scmoutput
    CALL SCMoutput(scmout3,                                             &
         'frad_lw','Net LW radiative flux in BL','Km/s',                &
