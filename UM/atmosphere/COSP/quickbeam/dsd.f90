...
! ----- INTERNAL -----
  ...
  CHARACTER(LEN=*),PARAMETER :: routine_name='DSD'
...
! :: vu = distribution width parameter
  ...
  case(1)  
    ...
    tmp1 = (Q*1E-3)**(1./bpm)
        ...
        icode = 9
        cmessage = 'No default value for Dm or Np provided!'
        CALL cosp_ereport(routine_name,cmessage,icode)
      endif
...
! :: dmax_mm = upper bound (mm)
  ...
  case(3)
    ...
    if(Re>0) then
      icode = 9
      cmessage = 'Variable Re not supported for Power-Law distribution'
      CALL cosp_ereport(routine_name,cmessage,icode)
    elseif(Np>0) then
      icode = 9
...
!     // Np, log_sigma_g are given    
    ...
    exp((-0.5*(log(0.5*D/rg)/log_sigma_g)**2.)) &
      ...
      icode = 9
      cmessage = 'Must specify a value for sigma_g'
      CALL cosp_ereport(routine_name,cmessage,icode)
    endif
