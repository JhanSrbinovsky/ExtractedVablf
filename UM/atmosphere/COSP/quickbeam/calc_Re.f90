...
! ----- INTERNAL -----
  ...
  CHARACTER(LEN=*),PARAMETER :: routine_name='calc_Re'
...
! :: vu = distribution width parameter 
  ...
  case(1)  
    ...
    if(Np>1E-30) then
      ...
      icode = 9
      cmessage = 'Must specify a value for Np in each volume '//               &
    ...
    elseif (abs(local_p3+1) > 1E-8) then
      ...
      ! vu isn't specified
      icode = 9
      cmessage = 'Must specify a value for vu for Modified Gamma distribution'
      CALL cosp_ereport(routine_name,cmessage,icode)
    ...
    D0 = gamma(vu)/gamma(vu+1)*dm
        ...
        if(Np.eq.0) then
            ...
            if( abs(p1+1) > 1E-8 ) then  !   use default number concentration 
              ...
              icode = 9
              cmessage =  'Must specify Np or default value (p1=Dm [um] '//    &
                          'or p2=Np [1/kg]) for modified Gamma distribution'
              CALL cosp_ereport(routine_name,cmessage,icode)
            endif
        else
            local_Np=Np;    
...
! :: ld = slope parameter (um)
  ...
  case(2)
    ...
    elseif (abs(p2+1) > 1E-8) then  ! lambda=ld has been specified as default
      ...
      icode = 9
      cmessage =  'Must specify Np or default value (p1=No or p2=lambda) '//   &
...
! :: dmax_mm = upper bound (mm)
  ...
  case(3)
    ...
    if(Np>0) then
      icode = 9
      cmessage =  'Variable Np not supported for Power Law distribution'
      CALL cosp_ereport(routine_name,cmessage,icode)
    endif
! ---------------------------------------------------------!
! // monodisperse                                          !
...
! :: D0 = particle diameter (um) == Re
  ...
  case(4)
     ...
     if(Np>0) then
       icode = 9
       cmessage = 'Variable Np not supported for Monodispersed distribution'
       CALL cosp_ereport(routine_name,cmessage,icode)
     endif
...
! :: log_sigma_g = ln(geometric standard deviation)
  ...
  case(5)
        ...
        if( abs(local_p3+1) > 1E-8 ) then
          ...
          icode = 9
          cmessage = 'Must specify a value for sigma_g when using a '//        &
    ...
    if( Np.eq.0 .and. (abs(p2+1) > 1E-8) ) then ! use default value of rg
            ...
            elseif(abs(p2+1) < 1E-8) then
                ...
                icode = 9
                cmessage = 'Must specify Np or default value '//               &
