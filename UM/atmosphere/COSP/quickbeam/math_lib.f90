...
!    Output, real ( kind = 8 ) RESULT, the approximate value of the integral.
  ...
  CHARACTER(LEN=*),PARAMETER :: routine_name='AVINT'
  ...
  if ( ntab < 3 ) then
    icode = 9
    write(cmessage,*) 'NTAB is less than 3. NTAB = ', ntab
  ...
  if (lerror) then
    icode = 9
    cmessage = 'XTAB(I) is not greater than XTAB(I-1)'
    CALL cosp_ereport(routine_name,cmessage,icode)
  end if
