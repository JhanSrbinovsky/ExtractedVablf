  subroutine radar_simulator( &
    hp, &								! hydrometeor and radar simulator properties 
    nprof,ngate, &							! size of input matricies
    undef, &             ! value for missing data
    hgt_matrix,hm_matrix,re_matrix,Np_matrix, &				! inputs
    p_matrix,t_matrix,rh_matrix, &					
    Ze_non,Ze_ray,a_to_vol,g_to_vol,dBZe, &			! outputs
    g_to_vol_in,g_to_vol_out)						! optional in/out
...
! ----- INTERNAL -----
  ...
  CHARACTER(LEN=*),PARAMETER :: routine_name='radar_simulator'
...
!DEPENDS ON: calc_Re
            ...
            if (Np_matrix(tp,pr,k)>0) then
                icode = -9
                cmessage = 'Re and Np set for the same volume&hydrometeor '//  &
                           'type. Np is being ignored.'
...
! DEPENDS ON: zeff
            ...
            if(Np_matrix(tp,pr,k)>0 .and. DO_NP_TEST ) then
              ...
              if( (Np_matrix(tp,pr,k)-Np)/Np_matrix(tp,pr,k)>0.1 ) then
                icode = 9
                cmessage = 'Np input does not match sum(N).'
                CALL cosp_ereport(routine_name,cmessage,icode)
              endif
            endif
            ...
            if ( hp%Z_scale_flag(tp,itt,iRe_type) .and. DO_LUT_TEST )  then
              ...
              if ( abs(10*log10(ze) - 10*log10(hp%Ze_scaled(tp,itt,iRe_type) * &
                   scale_factor)) > 2 ) then
                icode = 9
                cmessage = 'More than 2 dBZe difference.'
                CALL cosp_ereport(routine_name,cmessage,icode)
              endif
            endif
...
!     :: volume is hydrometeor-free	
      ...
      ! in the new scheme I assign have the attenuation to the current layer
      if(d_gate==1) then
