...
#if defined(SCMA)
      Real                                                              &
     &  LWP(qdims%i_start:qdims%i_end,qdims%j_start:qdims%j_end)        &
                                      ! Liquid water path (kg/m2)
     &, IWP(qdims%i_start:qdims%i_end,qdims%j_start:qdims%j_end)        &
                                      ! Ice water path    (kg/m2)
     &, bl_alltypes(pdims%i_start:pdims%i_end,pdims%j_start:pdims%j_end)&
                                      ! Boundary layer all types
     &, rho_theta_levs                ! Rho averaged onto theta levels
#endif
!
...
! off, if it can't.
      ...
      su10=.true.               ! Calculate u10m
      sv10=.true.               !    "      v10m
      slh=.true.                !    "      latent_heat
      sq1p5 =.true.             !    "      q1p5m
      st1p5=.true.              !    "      t1p5m
      simlt=.true.              !    "      sice_mlt_htf
      smlt=.true.               !    "      snomlt_surf_htf
      sq_T1p5=.false.           ! This can cause diagnostics_bl to
...
!     Set single diagnostic of BL type.
      Do j = pdims%j_start,pdims%j_end
        Do i = pdims%i_start,pdims%i_end
          bl_alltypes(i,j) = bl_type_1(i, j)*1.0                        &
     &                     + bl_type_2(i, j)*2.0                        &
     &                     + bl_type_3(i, j)*3.0                        &
     &                     + bl_type_4(i, j)*4.0                        &
     &                     + bl_type_5(i, j)*5.0                        &
...
!     Calculate density averaged on theta levels, IWP and LWP
     ...
     &              /(r_rho_levels(i,j,k+1)*r_rho_levels(i,j,k+1)) )
            LWP(i,j) = LWP(i,j) + qcl_latest(i,j,k)*rho_theta_levs*     &
     &                 (r_rho_levels(i,j,k+1)-r_rho_levels(i,j,k))
            IWP(i,j) = IWP(i,j) + qcf_latest(i,j,k)*rho_theta_levs*     &
     &                 (r_rho_levels(i,j,k+1)-r_rho_levels(i,j,k))
          End Do ! i
        End Do ! j
      End Do ! k
...
! DEPENDS ON: dgnstcs_imp_ctl2
     ...
     &     pdims%k_end,qdims%k_end,bl_levels,                           &
     &     cloud_levels,offx ,offy ,                                    &
     &     rhcrit,L_murk,combined_cloud,nclds,cumulus,ntml,             &
     &     plsp,conv_rain,conv_snow,ls_rain,ls_snow,                    &
     &     n_rows,R_u,R_v,u_incr_diag_bl,v_incr_diag_bl,                &
     &     zh,zht,bl_type_1,bl_type_2,bl_type_3,bl_type_4,              &
     &     bl_type_5,bl_type_6,bl_type_7,bl_alltypes,                   &
     &     fqt,ftl,T_latest,q_latest,cf_latest,cfl_latest,cff_latest,   &
     ...
     &     p_star,u10m,v10m,latent_heat,cca_at_base,                    &
