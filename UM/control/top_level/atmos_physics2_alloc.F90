...
INTEGER, ALLOCATABLE :: tile_index(:,:),tile_pts(:)
...
ALLOCATE(g_leaf(land_points,npft))
         ! Leaf turnover rate (/360days)
ALLOCATE(gpp_ft(land_points,npft))
         ! Gross primary productivity on PFTs (kg C/m2/s)
ALLOCATE(npp_ft(land_points,npft))
         ! Net primary productivity (kg C/m2/s)
ALLOCATE(resp_p_ft(land_points,npft))
