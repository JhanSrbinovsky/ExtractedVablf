...
! Additional variable for the SCM diagnostics
        ...
        nconvars, ntrad1, conv_mode, l_emcorr_opt,                      &
#endif
! error information
        Error_code  )
...
! Boundary Layer output arrays
     ...
     REAL :: u_incr_diag_bl        (udims%i_start:udims%i_end,         &
                                    udims%j_start:udims%j_end,         &
                                    udims%k_start:udims%k_end)
     REAL :: v_incr_diag_bl        (vdims%i_start:vdims%i_end,         &
                                    vdims%j_start:vdims%j_end,         &
                                    vdims%k_start:vdims%k_end)
     REAL :: T_incr_diag_bl        (tdims%i_start:tdims%i_end,         &
                                    tdims%j_start:tdims%j_end,         &
                                                1:tdims%k_end)
     REAL :: q_incr_diag_bl        (qdims%i_start:qdims%i_end,         &
                                    qdims%j_start:qdims%j_end,         &
                                                1:qdims%k_end)
     REAL :: qcl_incr_diag_bl      (qdims%i_start:qdims%i_end,         &
                                    qdims%j_start:qdims%j_end,         &
                                                1:qdims%k_end)
     REAL :: qcf_incr_diag_bl      (qdims%i_start:qdims%i_end,         &
                                    qdims%j_start:qdims%j_end,         &
                                                1:qdims%k_end)
     REAL :: bulk_cf_incr_diag_bl  (qdims%i_start:qdims%i_end,         &
                                    qdims%j_start:qdims%j_end,         &
                                                1:qdims%k_end)
     REAL :: cf_liquid_incr_diag_bl(qdims%i_start:qdims%i_end,         &
                                    qdims%j_start:qdims%j_end,         &
                                                1:qdims%k_end)
     REAL :: cf_frozen_incr_diag_bl(qdims%i_start:qdims%i_end,         &
                                    qdims%j_start:qdims%j_end,         &
                                                1:qdims%k_end)
...
! diagnostics out - initial values will then need to be restored.
      ...
      IF (conv_mode == 1      .AND.                                     &
          a_step    >= ntrad1 .AND.                                     &
          MOD(a_step-ntrad1, a_sw_radstep_diag) == 0) THEN
...
! DEPENDS ON: save_conv
     ...
     &                 nitr_acc, nitr_diss,                             &
     &                 co2, free_tracers, ukca_tracers, OZONE_TRACER,   &
...
! DEPENDS ON: sub_data
          Call SUB_DATA(                                                &
            ...
            t_surf, zh, z0msea, cca, ccb, cct, soil_layer_moisture)
        End if ! conv_mode
...
! only call on 1st cycle or if not fast running
     IF ( cycleno == 1 .OR. .NOT. l_quick_ap2 ) THEN
        ...
        i_field = i_field + 1
        fields_to_swap(i_field) % field       => rhogamu(:,:,2:)
        fields_to_swap(i_field) % field_type  =  fld_type_p
        ...
        i_field = i_field + 1
        fields_to_swap(i_field) % field       => rhogamv(:,:,2:)
        fields_to_swap(i_field) % field_type  =  fld_type_p
        fields_to_swap(i_field) % levels      =  bl_levels-1
        fields_to_swap(i_field) % rows        =  rows
        fields_to_swap(i_field) % vector      =  .FALSE.
...
#if defined(A03_1A)
ALLOCATE(rhogamu_u(udims%i_start:udims%i_end,udims%j_start:udims%j_end,&
                    2:bl_levels))
ALLOCATE(rhogamv_v(vdims%i_start:vdims%i_end,vdims%j_start:vdims%j_end,&
                    2:bl_levels))
...
ALLOCATE(f_ngstress_v(vdims%i_start:vdims%i_end,                    &
...
#if defined(A03_1A)
        CALL p_to_u(rhogamu(pdims_s%i_start,pdims_s%j_start ,2),    &
                    pdims_s%i_start,pdims_s%i_end,                  &
                    pdims_s%j_start,pdims_s%j_end,                  &
                    udims%i_start,udims%i_end,                      &
                    udims%j_start,udims%j_end,                      &
                    1,bl_levels-1,                                  &
                    rhogamu_u(udims%i_start,udims%j_start,2))

        CALL p_to_v(rhogamv(pdims_s%i_start,pdims_s%j_start ,2),    &
                    pdims_s%i_start,pdims_s%i_end,                  &
                    pdims_s%j_start,pdims_s%j_end,                  &
                    vdims%i_start,vdims%i_end,                      &
                    vdims%j_start,vdims%j_end,                      &
                    1,bl_levels-1,                                  &
                    rhogamv_v(vdims%i_start,vdims%j_start,2))
        ...
        DO k = 2, bl_levels
           DO j = 1, rows
              DO i = 1, row_length
! for some reason rhokm is defined 0->bl_levels-1
                 rhokm_v(i,j,k) = rhokm(i,j,k-1)
                 rhokm_u(i,j,k) = rhokm(i,j,k-1)
                 IF(formdrag ==  explicit_stress)THEN
