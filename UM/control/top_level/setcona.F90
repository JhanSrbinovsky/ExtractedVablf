...
!LL      modules and the argument list in arglndm.
!LL
!LL Method:
...
USE dynamics_input_mod, ONLY:   L_mix_ratio,L_LBC_balance,L_lbc_new,    &
                                L_transparent,L_regular,n_rims_to_do,   &
...
USE dynamics_testing_mod, ONLY:                                         &
                ...
                phi_frac,lam_frac,lam_var, phi_var,                     &
...
!----------------------------------------------------------------------
      ...
      IF (lhook) CALL dr_hook('SETCONA',zhook_in,zhook_handle)
...
!  Set processor mpp properties in module to avoid  
!  having to include in argument list from calling routines
...
! DEPENDS ON: swap_bounds
      CALL swap_bounds(exner_rho_levels, row_length, rows,              &
                       model_levels+1, offx, offy, fld_type_p, .FALSE.)
...
! Allocate full size arrays to avoid out-of-bounds in atm_step 
      ...
      If ( L_regular ) Then 
        ...
        Write ( Unit = 6, fmt=*) ' '
        WRITE (6,'(A)') '***************** The horizontal '//        &
            'grid is regular: l_regular = T ****************'
        Write ( Unit = 6, fmt=*) ' '
...
! Allocate arrays for dyn_var_res_mod module
        ...
        If (.not.allocated(recip_dphip)) Then
          Allocate (recip_dphip ( 1, 1 ))
        End If
        If (.not.allocated(recip_dphiv)) Then
          Allocate (recip_dphiv ( 1, 1 ))
        End If
      ...
      Else  !  Variable grid; set up parameters
        ...
        Write ( Unit = 6, fmt=*) ' '
        WRITE (6,'(A)') '************ This run uses a '//               &
            'variable horizontal grid: l_regular = F ***********'
        Write ( Unit = 6, fmt=*) ' '
...
! Allocate arrays for dyn_var_res_mod module
     ...
     &                               1-halo_j : n_rows+halo_j ))
        ...
        IF (.NOT.ALLOCATED(look_lam)) THEN
          ALLOCATE (look_lam(max_look))
        END IF
        IF (.NOT.ALLOCATED(look_phi)) THEN
          ALLOCATE (look_phi(max_look))
        END IF    
...
!     resolution models.
      ...
      DO k=1,rows
        DO j=1,row_length 
          xscale(j,k)=delta_lambda*earth_radius*cos_theta_latitude(j,k)
        END DO
      END DO
...
! ----------------------------------------------------------------------
     ...
     & solver is ACTIVE ***'
      ...
      IF (lhook) CALL dr_hook('SETCONA',zhook_out,zhook_handle)
      RETURN
